...
async function userDraftPlayer() {
    if (await authenticateSession() == true) {
        await checkToClearDraftLog();
        let nextDraftPickSpan = document.getElementById("nextDraftPick");
        ...
        let res = await fetch("http://localhost:80/api/teams/userDraftPlayer/?username=" + getCookie("username") + "&playerIndex=" + playerToDraftIndex, {
            method: 'POST',
        });
        let data = await res.json();
        console.log(data);
